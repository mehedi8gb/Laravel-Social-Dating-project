    ...
    <div class="row">
      <div class="col-lg-12">
        <div class="w-100 text-center py-4 img-profile-user">
          ...
          <div class="text-center position-relative avatar-wrap shadow @if (auth()->check() && auth()->user()->id != $user->id && Cache::has('is-online-' . $user->id) && $user->active_status_online == 'yes' || auth()->guest() && Cache::has('is-online-' . $user->id) && $user->active_status_online == 'yes') user-online-profile overflow-visible @elseif (auth()->check() && auth()->user()->id != $user->id && !Cache::has('is-online-' . $user->id) && $user->active_status_online == 'yes' || auth()->guest() && !Cache::has('is-online-' . $user->id) && $user->active_status_online == 'yes') user-offline-profile overflow-visible @endif">
          ...
          @endif
            ...
            @if (auth()->check() && auth()->user()->id != $user->id && $user->updates()->count() <> 0 && $settings->disable_tips == 'off')
            ...
            @endif
              ...
              @if ($user->website != '')
                <div class="d-block mb-1">
                  <a href="{{$user->website}}" title="{{$user->website}}" target="_blank" class="text-muted share-btn-user text-truncate"><i class="fa fa-link mr-1"></i> {{Helper::removeHTPP($user->website)}}</a>
                </div>
              @endif
