<header>
	<nav class="navbar navbar-expand-lg navbar-inverse fixed-top p-nav @if(auth()->guest() && request()->path() == '/') scroll @else p-3 @if (request()->is('messages/*')) d-none d-lg-block shadow-sm @elseif(request()->is('messages')) shadow-sm @else shadow-custom @endif {{ auth()->check() && auth()->user()->dark_mode == 'on' ? 'bg-white' : 'navbar_background_color' }} link-scroll @endif">
			...
			@endif
				...
				<li class="nav-item dropdown d-lg-block d-none">
					<a href="{{url('messages')}}" class="nav-link px-2" title="{{ trans('general.messages') }}">
						...
						<span class="notifications noti_msg notify @if (auth()->user()->messagesInbox() != 0) d-block @endif">
							{{ auth()->user()->messagesInbox() }}
                        {{-- {{ app\Models\User::where('id', Auth::user()->messagesNotify()->from_user_id)->first()->name }}
                        {{ Auth::user()->messagesNotify()->message }} --}}
							</span>
<input type="hidden" class="notifications noti_msg" name="message_count" value="{{ auth()->user()->messagesInbox() }}">
