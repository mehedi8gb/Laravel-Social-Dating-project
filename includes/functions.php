	...
	*/
	...
	public function iN_PaymentsSubscriptionsList($userID, $paginationLimit, $page) {
		...
		if ($this->iN_CheckUserExist($userID) == 1) {
			...
			WHERE S.status IN('inactive','active') AND S.subscribed_iuid_fk = '$userID' ORDER BY S.subscription_id DESC LIMIT $start_from, $paginationLimit
			") or die(mysqli_error($this->db));
			while ($row = mysqli_fetch_array($query)) {
				$data[] = $row;
			}
	...
	public function iN_CalculateCurrentMonthEarning($userID) {
		$userID = mysqli_real_escape_string($this->db, $userID);
		if ($this->iN_CheckUserExist($userID) == 1) {
			$query = mysqli_query($this->db, "SELECT SUM(user_net_earning) AS calculate FROM i_user_subscriptions WHERE subscribed_iuid_fk = '$userID' AND status IN('active','inactive') AND MONTH(created)= MONTH(CURDATE())") or die(mysqli_error($this->db));
			$result = mysqli_fetch_array($query, MYSQLI_ASSOC);
			return $result;
		} else {
			return false;
