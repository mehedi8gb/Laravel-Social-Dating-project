    ...
    $path = config('path.messages');
     ...
     if ($settings->currency_position == 'right') {
      ...
      // Setup the validator
        ...
        //   $message = User::where('id', Auth::user()->messagesNotify()->from_user_id)->first()->avatar;
          $messageData = trim(Helper::checkTextDb($request->get('message')));
          event(new MessageNotifyEvent($userData, $messageData));
