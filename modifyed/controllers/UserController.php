...
public function notificationSettingPage(){
    return view('users.settings.notifications_setting');
}
	...
	Validator::extend('letters', function($attribute, $value, $parameters){
      ...
      $validator = Validator::make($input, [
        ...
        'email'  => 'required_if:is_admin,==,admin|unique:users,email,'.$id,
        'website' => 'url',
        'facebook' => 'url',
        'twitter' => 'url',
        'instagram' => 'url',
...
// mehedi
    public function ajaxMessages()
    {
        ...
        // Messages
        if (auth()->user()->messagesInbox() ) {
            $userData = User::where('id', Auth::user()->messagesNotify()->from_user_id)->first();
            $name = $userData->name;
            $avatar = $userData->avatar;
            $data = Auth::user()->messagesNotify()->message;
            $username = $userData->username;
            $id = $userData->id;

            $session = Carbon::parse(Auth::user()->messagesNotify()->created_at)->diffInSeconds();

            if ($session < 10) {
                return response()->json([
                    'data' => $data,
                    'avatar' => 'https://naughtymedia.sfo3.digitaloceanspaces.com/uploads/avatar/'.$avatar,
                    'name' => 'You Have New Message From '.$name,
                    'link' => url('messages').'/'.$id.'/'.$username
                ]);
            } else {
